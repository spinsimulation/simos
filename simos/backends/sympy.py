...
def ket2dm(ket):
    if isket(ket):
        return QSMatrix(ket * ket.H, dims=[ket.shape[0], ket.shape[0]])
    if isbra(ket):
        return QSMatrix(ket.H * ket, dims=[ket.shape[1], ket.shape[1]])
    else:
...
def sqrt_fun(input):
    if isinstance(input, (_sp.MutableDenseNDimArray,_sp.Matrix)):
...
def multelem_fun(arg1, arg2):
    ...
    for ind_i in range(len(a)):
        a[ind_i] = a[ind_i]*b[ind_i]
    return a
