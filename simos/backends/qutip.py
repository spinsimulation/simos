...
def directsum(operator, pre, post):
    ...
    M = _np.zeros((dimtot, dimtot), dtype = complex)
    M[pre:pre+operator.shape[0], pre:pre+operator.shape[1]] = operator.full()
    M = _qu.Qobj(M)   
    return M 
...
# def applyFockeroperator(F,rho,weights=None):
#     if weights is None:
...
#     # or a focker-Hilbert space propagator.
#     # or a focker-Liouville space propagator.
...
#     else:
#         # Focker-space operator
#         dof = F_dims[0][0]
#         if len(F) != 1:
#             raise ValueError("Focker space propagator must be a single operator.")
